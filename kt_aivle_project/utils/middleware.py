...
class MoveJWTCookieIntoTheBody(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, *view_args, **view_kwargs):
        if request.path == '/userapp/token/verify/' and api_settings.JWT_AUTH_COOKIE in request.COOKIES:
...
class MoveJWTRefreshCookieIntoTheBody(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, *view_args, **view_kwargs):
        ...
        if request.path == '/userapp/token/refresh/' and api_settings.JWT_AUTH_REFRESH_COOKIE in request.COOKIES:
            if request.body != b'':
                data = json.loads(request.body)
